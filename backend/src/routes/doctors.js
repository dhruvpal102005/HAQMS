...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to retrieve doctors directory' });
  }
});

// GET /api/doctors/stats
// Returns aggregation details about available doctors
// PERFORMANCE BUG FIXED: Parallel queries using Promise.all
router.get('/stats', authenticate, async (req, res) => {
  try {
    const start = Date.now();

    // Independent database calls are run in parallel, avoiding event loop blockage
